...
const {setGearPosition,setLightStatus } = useDashboardStore()
  ...
  const updateFiveIndexOfData = () => {
    writeData.value[5] = writeData.value[1] ^ writeData.value[2] ^ writeData.value[3] ^ writeData.value[4] ^ writeData.value[6] ^ writeData.value[7] ^ writeData.value[8] ^ writeData.value[9] ^ writeData.value[10] ^ writeData.value[11];
  }
