...
const initChart = () => {
  return new Promise((resolve, reject) => {
    try {
      ...
      y2 = y1;
      path = `M ${x1} ${y1} A${radius} ${radius} 0 1 0 ${x2} ${y2} L${chartSize / 2} ${chartSize / 2}`;
...
const renderChart = (value: number) => {
  ...
  // 计算终止角度
  const angle = (220 * (value - minSpeed) / (maxSpeed - minSpeed) - 200) * Math.PI / 180;
  // 开始作画
  // context?.clearRect(0, 0, chartSize, Math.round(chartSize * 0.7))
  // eslint-disable-next-line no-self-assign
  canvasEl.width = canvasEl.width
  context?.beginPath();
  const gradient = context?.createRadialGradient(chartSize / 2, chartSize / 2, 0, chartSize / 2, chartSize / 2, radius);
  ...
  context.fill();
  // 画5条分割线
  for (let i = 1; i <= 5; i++) { // 所有线条的起点都是画布的中心位置
    // 计算终点
...
watch(() => props.isKmUnit, async (value) => {
